...
DESCRIPTION:
   Reference IBitmap implementation for 24-bit display.  This file
...
#define COLOR_DEPTH  24 // number of significant bits per pixel, returned in AEEBitmapInfo struct
#define PIXEL_SIZE   32 // total number of bits per pixel, including padding (DIB-compatible bitmaps only)
#define COLOR_SCHEME IDIB_COLORSCHEME_888  // DIB color scheme (DIB-compatible bitmaps only)
...
#define RGB_TO_NATIVE(r, g, b) \
   ((NativeColor)(((r) << 16) | ((g) << 8) | (b)))

#define NATIVE_TO_RGB(c) \
   ((RGBVAL)HTONL(c))
   //((RGBVAL)( ((c & 0xFF0000) >> 8) | ((c & 0xFF00) << 8) | ((c & 0xFF) << 24) ))
...
#define BITSPREAD_MASKLOW               0x00FF00FFU
#define BITSPREAD_MASKHIGH              0x000000FFU
#define BITSPREAD_BIASLOW               0x0
#define BITSPREAD_BIASHIGH              0x0
#define BITSPREAD_EXTRABITS             0x8
#define BITSPREAD_SCALEALPHA(a)         (a)
...
#define BITSPREAD_TONATIVE_MASK(dst, src, ml, mh) \
   dst = ((((src.low) & (ml)) | ((src.high) & (mh))<<8 )); 

#define GEN_BITSPREAD_FROMRGB(dst, r,g,b)\
   dst.low = ((r)<<16 | (b)) & BITSPREAD_MASKLOW; \
   dst.high = ((g)) & BITSPREAD_MASKHIGH;
